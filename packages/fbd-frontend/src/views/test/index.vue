    ...
    <div class="range flex justify-between">
      ...
      >
        <div class="range-item-icon">
          ...
          <div
            v-show="state.currentStatus !== 'all'"
            class="range-item-affix"
          />
        </div>
      </div>
    </div>
...
import {
  timeZoneUnit2, isNumber, fmtPayRate, numWithCommas, copyByText,
...
export default {
  ...
  setup() {
    ...
    const isListEmpty = computed(() => !state.betRecordData?.length && !state.isListLoading && !state.isLoading);
    ...
    const getData = async () => {
      ...
      state.pageData.isFirstPage = first;
...
.betrecord {
  ...
  &-sum {
    @apply fixed left-0 w-full mb-0 py-2 px-5 bg-secondary-content;
