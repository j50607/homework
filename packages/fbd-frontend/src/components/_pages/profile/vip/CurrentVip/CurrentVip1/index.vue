...
export default {
  ...
  setup(props, { emit }) {
    ...
    const nextLevelBetSumPercentage = computed(() => {
      ...
      if (realNextLevelBetSumPercentage && realNextLevelBetSumPercentage < 5) {
        return 5;
    ...
    const progress1 = computed(() => {
      const holdProgress = NP.divide(NP.times(nextLevelBetSumPercentage.value, 33.3333), 100);
      if (holdProgress && holdProgress < 3) {
        return 3;
      }
      return holdProgress;
    });
    const progress2 = computed(() => {
      const depositProgress = NP.divide(NP.times(nextLevelDepositSumPercentage.value, 33.3333), 100);
      if (depositProgress && depositProgress < 3) {
        return 3;
      }
      return depositProgress;
    });
    const progress3 = computed(() => {
      const betProgress = NP.divide(NP.times(nextLevelTotalBetSumPercentage.value, 33.3333), 100);
      if (betProgress && betProgress < 3) {
        return 3;
      }
      return betProgress;
    });
